    ...
    override fun getView(position: Int, convertview: View?, p2: ViewGroup?): View {
        ...
        database.child("DanhGia").child(arrayList.get(position).key).addChildEventListener(object : ChildEventListener{
            ...
            override fun onChildChanged(p0: DataSnapshot?, p1: String?) {
                ...
                //
                var tb = Math.round((sum/ ListRT!!.size)*10)/10.toFloat()
        ...
        arrayList.get(position).SoCmmt=0
        arrayList.get(position).LuocLike = 0
        ...
        if (doctaikhoan()){
            database.child("Like").child(arrayList.get(position).key).child(id_USER).addValueEventListener(object :ValueEventListener{
                override fun onCancelled(p0: DatabaseError?) {
                }
