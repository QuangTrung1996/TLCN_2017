...
class ScheduleFragment : Fragment() {
    ...
    private val sdfDate : SimpleDateFormat = SimpleDateFormat("dd-MM-yyyy", Locale.getDefault())
    private val sdfTime : SimpleDateFormat = SimpleDateFormat("HH:mm", Locale.getDefault())
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        lvLich.onItemLongClickListener = AdapterView.OnItemLongClickListener{ _, _, position, _ ->
...
//                val dateKT = java.sql.Date(arr[i*5+3].toLong())
                ...
                if (sdfTime.format(timeBD) == "00:00" && sdfTime.format(timeKT) == "23:59"){
                    mangLich.ngayBD  = "Cả ngày"
                    mangLich.ngayKT  = ""
                }
                else {
                    if (sdfTime.format(timeBD) == "00:00"){
                        mangLich.ngayBD  = "Ngày mới"
                        mangLich.ngayKT  = sdfTime.format(timeKT)
                    }
                    else {
                        if (sdfTime.format(timeKT) == "23:59"){
                            mangLich.ngayBD  = sdfTime.format(timeBD)
...
//        }
    ...
    private fun getEvents(timeInMillisBD: Long, timeInMillisKT: Long, ten: String, note: String, ma: String): List<Event> {
        ...
        val date1 : Date = sdf.parse(sdfDate.format(dateBD).toString() + " at " + "23:59")
        val date2 : Date = sdf.parse(sdfDate.format(dateKT).toString() + " at " + "0:0")
        ...
        if (noDay.toInt() == 0) {
            ...
            if (noDay.toInt() == 1) {
                ...
                for (i in 1..(noDay-1)){
                    ...
                    // thoi gian bat dau va ket thuc cua 1 ngay trong 1 su kien nhieu ngay : bat dau luc 0h va ket thuc luc 23:59
                    dateTimeBD = sdf.parse(sdfDate.format(date).toString() + " at " + "0:0")
                    dateTimeKT = sdf.parse(sdfDate.format(date).toString() + " at " + "23:59")
